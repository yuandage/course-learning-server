...
public interface CourseDao extends JpaRepository<Course, String> {
	...
	@Query(value = "SELECT * FROM course WHERE parent_id in ( SELECT id FROM course_classification where parent_id=:id )", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM course WHERE parent_id in ( SELECT id FROM course_classification where parent_id=:subjectId )",
			countQuery = "SELECT count(*) FROM course WHERE parent_id in ( SELECT id FROM course_classification where parent_id=:subjectId )", nativeQuery = true)
