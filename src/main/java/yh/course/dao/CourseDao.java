...
public interface CourseDao extends JpaRepository<Course, String>, JpaSpecificationExecutor<Course> {
    ...
    @Query(value = "SELECT id,name,summary,catalog,parent_id,create_date,update_date FROM course WHERE parent_id in ( SELECT id FROM subject where parent_id=:subjectId )",
